...
while(count($servedQuestions) <= 9) {
    $n1 = mt_rand(1,140);		//for random questions from 150 pool of questions
    if (in_array($n1, $servedQuestions)){ continue;
	//
	}
	$q = mysqli_query($connection,"select * from questions WHERE qno = '".$n1."'");
    $servedQuestions[] = $n1;
}
...
<body bgcolor="#FFFFFF" topmargin=5 leftmargin=5 marginheight=5 marginwidth=5>
 ...
 <form name="MainForm" method="post" action="result.php" onSubmit="return(ValidateForm());">
